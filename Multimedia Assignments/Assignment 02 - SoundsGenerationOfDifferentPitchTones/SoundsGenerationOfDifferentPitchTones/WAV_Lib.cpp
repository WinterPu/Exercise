...
WavFile::WavFile(int v_frequency, int v_volume, int v_durations) :
	m_samplefreq(44100), m_channels(2),m_channelbits(8),frequency(v_frequency),volume(v_volume),durations(v_durations){

	totalLen = (m_samplefreq * m_channels * m_channelbits / 8) * durations + 44;//文件总长度=(采样率 * 通道数 * 比特数 / 8) * 持续时间(s)
	wav_data_length = totalLen - 44;
	CreateWaveHeader();
	MakeWaveData(pHeader->pwf.wf.nSamplesPerSec, frequency, volume, pWaveBuffer + WAVE_HEAD_LENGTH, m_samplefreq*durations);//采样点数
}
